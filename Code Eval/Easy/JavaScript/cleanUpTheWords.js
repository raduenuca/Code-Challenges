(function() {
    ...
    var cleanUpTheWords = function(str) {
        str = str.toLowerCase();
        var captureWords = /.*?([a-z]+).*?/gi;
        var matches, result = [];

        while ((matches = captureWords.exec(str)) !== null) {
            if (matches.index === captureWords.lastIndex) {
                captureWords.lastIndex++;
            }
    ...
    var cleanUpTheWords_ = curry(map_, cleanUpTheWords);
