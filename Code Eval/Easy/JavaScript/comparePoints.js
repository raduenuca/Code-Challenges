(function() {
    ...
    var computeDeltas = function(directions){
        var deltaX = directions[2] - directions[0];
        var deltaY = directions[3] - directions[1];
        
        return sign(deltaX) + sign(deltaY);
    };
    
    var getDirections = function(key){
        var directions = [
            { key: "00", value: "here"},
            { key: "0+", value: "N"},
            { key: "0-", value: "S"},
        ...
        return find_(function(direction){
            return direction.key === key;
        }, directions).value;
    };
    
    var comparePoints = compose(
        getDirections,
        computeDeltas,
        convertToIntArray
    );

    var comparePoints_ = curry(map_, comparePoints);
