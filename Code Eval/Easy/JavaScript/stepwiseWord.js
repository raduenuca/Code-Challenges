(function() {
    ...
    var longestString = function(a, b) {
        return a.length >= b.length ? a : b;
    };

    var stepwiseWord = function(word) {
        var addSteps = function(letter, index){
            return Array(index + 1).join("*") + letter;
        };
        
        var convertToStepWise = compose(curry(map_, addSteps), letters);
        
        return convertToStepWise(word);
    };

    var longestWord= curry(reduce_, longestString, undefined);
    var longestWord_ = curry(map_, longestWord);
    var stepwiseWord_ = curry(map_, stepwiseWord);
